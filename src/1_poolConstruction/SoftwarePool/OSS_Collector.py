...
def hashing(repoPath, repoName, commit):
	...
	resDict  = {}
	repoMetaPath = metaPath + repoName

	try:
		os.mkdir(repoMetaPath)
	except:
		pass
	...
	for path, dir, files in os.walk(repoPath):
		for file in files:
			...
			if file.lower() in metas:
				...
				if path == repoPath: #IF ROOT
					newMetaName = newMetaName + '_[ROOT]'
				else:
					newMetaName = newMetaName + '_[NOROOT]'
				
				shutil.copy(filePath, newMetaName)
				...
				try:
					...
					for i in allFuncs:
						...
						if i != '' and len(elemList) >= 8 and func.fullmatch(elemList[3]):
							...
							fraw = open(funcPath + repoName + '/' + funcHash, 'w')
							fraw.write(nocoFuncBody)
							fraw.close()
